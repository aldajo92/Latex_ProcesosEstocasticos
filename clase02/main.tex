...
\begin{center}
\begin{tikzpicture}
  % Ovalo irregular alrededor del teorema
  \draw[thick, blue, rounded corners] (-1,-0.5) rectangle (8,1.5);
  \node at (3.5,0.5) {$\text{Bin}(n, p(n)) \;\;\longrightarrow\;\; \text{Poisson}(\lambda)$};
\end{tikzpicture}
\end{center}
...
\begin{equation*}
n \, p(n) \;\;\longrightarrow\;\; \lambda.
\end{equation*}

\textbf{Teorema}: Las definiciones (1) y (2) son equivalentes:
...
\begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=3cm,
  ...
  \node[state] (1) [above]        {1};
  \node[state] (2) [below left]   {2};
  \node[state] (3) [below right]  {3};
