...
\section*{Probabilidades y tiempos de Absorción}

Considere una carrera de 2 años:
\begin{itemize}
    \item A: Primer año
    \item 2: Segundo año  
    \item G: Graduado
...
\tikzstyle{every state}=[fill=white,draw=black,text=black,minimum size=25pt]
  ...
  % Estados: 1 y 2 arriba, E y G abajo
  \node[state,dashed] (1) at (0,0) {1};
  \node[state,dashed] (2) at (4,0) {2};
  \node[state] (E) at (0,-3) {E};
  \node[state] (G) at (4,-3) {G};

  % Transiciones desde estado 1
  \path (1) edge[loop above] node{$0.25$} (1)
            edge node[above]{$0.6$} (2)
            edge node[right, pos=0.7]{$0.15$} (E);
  
  % Transiciones desde estado 2
  \path (2) edge[loop above] node{$0.2$} (2)
            edge node[left, pos=0.7]{$0.7$} (G)
            edge node[right]{$0.1$} (E);
        
  % Auto-loops en estados absorbentes
  \path (G) edge[loop below] node{$1$} (G);
  \path (E) edge[loop below] node{$1$} (E);
...
\begin{enumerate}[label=\alph*)]
    \item ¿Cuál es la probabilidad de que un estudiante eventualmente se gradúe?
    \item ¿Y de que eventualmente sea expulsado?
\end{enumerate}

\textbf{Solución:}

Estados transitorios: $T = \{1, 2\}$

Estados absorbentes: $A = \{G, E\}$

La matriz se puede reorganizar como:
...
Sea $h(x)$ la probabilidad de que el estudiante estando en $x$ eventualmente se gradúe y $e(x)$ la probabilidad de que eventualmente sea expulsado estando en $x$.

Probabilidades a un paso:
\begin{align*}
h(1) &= P(1,1)h(1) + P(1,2)h(2) + P(1,G)h(G) + P(1,E)h(E) \\
h(2) &= P(2,1)h(1) + P(2,2)h(2) + P(2,G)h(G) + P(2,E)h(E)
\end{align*}

\begin{align*}
h(1) &= 0.25h(1) + 0.6h(2) + 0 \cdot 1 + 0.15 \cdot 0 = 0.25h(1) + 0.6h(2) \\
h(2) &= 0 \cdot h(1) + 0.2h(2) + 0.7 \cdot 1 + 0.1 \cdot 0 = 0.2h(2) + 0.7
\end{align*}

Por otro lado:
\begin{align*}
e(1) &= P(1,1)e(1) + P(1,2)e(2) + P(1,G)e(G) + P(1,E)e(E) \\
e(2) &= P(2,1)e(1) + P(2,2)e(2) + P(2,G)e(G) + P(2,E)e(E)
\end{align*}

Esto da:
\begin{align*}
e(1) &= 0.25e(1) + 0.6e(2) + 0 \cdot 0 + 0.15 \cdot 1 = 0.25e(1) + 0.6e(2) + 0.15 \\
e(2) &= 0 \cdot e(1) + 0.2e(2) + 0.7 \cdot 0 + 0.1 \cdot 1 = 0.2e(2) + 0.1
\end{align*}

Resolviendo el sistema:
\begin{align*}
h(1) &= 0.25h(1) + 0.6h(2) \Rightarrow (1-0.25)h(1) - 0.6h(2) = 0 \\
h(2) &= 0.2h(2) + 0.7 \Rightarrow (1-0.2)h(2) = 0.7
\end{align*}

De la segunda ecuación:
\begin{equation*}
e(1) = 0.25e(1) + 0.6e(2) + 0.15 \Rightarrow (1-0.25)e(1) - 0.6e(2) = 0.15
\end{equation*}

\begin{equation*}
e(2) = 0.2e(2) + 0.1 \qquad \Rightarrow (1-0.2)e(2) = 0.1
\end{equation*}
...
La matriz $(I-Q)^{-1} = (N-M) \times (N-M)$

La entrada $(i,j)$ de la matriz $(I-Q)^{-1} R$ es la probabilidad de que comenzando en el estado $i$ sea absorbido por el estado $j$.

\textbf{En general:} Sea $P$ una matriz de transición con $N$ estados, de los cuales $M$ son absorbentes y $N-M$ son transitorios, entonces siempre la matriz $P$ se puede arreglar de tal manera que:

\begin{equation*}
P = \begin{pmatrix}
Q_{(N-M) \times (N-M)} & R_{(N-M) \times M} \\
0_{M \times (N-M)} & I_{M \times M}
\end{pmatrix}
\end{equation*}
...
Sea $g(x)$ el tiempo esperado que un estudiante se gradúe o sea expulsado estando en el estado $x$.

Adelantando un paso:
\begin{align*}
g(1) &= P(1,1)g(1) + P(1,2)g(2) + P(1,G)g(G) + 1 \\
g(2) &= P(2,2)g(2) + P(2,G)g(G) + P(2,E)g(E) + 1
\end{align*}
...
g(1) \\
g(2)
\end{pmatrix} = (I-Q)^{-1} \begin{pmatrix}
1 \\
1
\end{pmatrix}
\end{equation*}
