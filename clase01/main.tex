...
Un \textbf{proceso estocástico} es una colección de variables aleatorias
\begin{equation*}
\{X_t\}_{t\in T},
\end{equation*}
...
\subsection*{Ejemplo: Articulos defectuosos}
\begin{enumerate}
...
Un proceso estocástico puede verse como una función
\begin{equation*}
...
\subsection*{Ejemplo: clientes en fila}
\begin{center}
...
\subsection*{Definición}
Un proceso $\{X_n\}_{n\ge 0}$ con espacio de estados $S$ es una \textbf{cadena de Markov} si para todo
...
\subsection*{Ejemplo: Matriz $2\times 2$ y grafo}
\begin{equation*}
P=\begin{pmatrix}
\alpha & 1-\alpha\\
\beta & 1-\beta
...
\subsection*{Ejemplo: Modelo de lluvia}
...
\subsection*{Ejemplo:}
Paseo aleatorio: Considere un proceso $\{X_n\}_{n\ge 0}$ con espacio de estados $S=\mathbb{Z}$ y sus probabilidades de transición están dadas por:
...
(E) edge[bend left] node{$1-p$} (D)
(E) edge[bend left] node{$p$} (F)
(F) edge[bend left] node{$1-p$} (E);
\end{tikzpicture}
\end{center}

% ===================== Ramificación =====================
\section{Proceso de ramificación}
Supóngase que un organismo tiene $j$ hijos con probabilidad $\alpha_j$:
\begin{equation*}
P(Z=j)=\alpha_j,
\end{equation*}
donde $Z$ es el número de hijos de un individuo.

\subsection*{Variables por generación}
\begin{itemize}
...
\subsubsection*{Diagrama de transición}
...
\textbf{Definición:}  
Las probabilidades de transición en $n$ pasos del estado $i$ al estado $j$ se denotan por:
\begin{equation*}
...
La \textbf{medida} $q(i) = P(X_0 = i)$ definida para todo $i \in S$ se conoce como \textbf{distribución inicial} de la cadena.
...
Un estado $j$ es \textbf{accesible} desde un estado $i$ si existe $n \geq 0$ tal que
...
Si $i \to j$ y $j \to i$ entonces se dice que $i$ y $j$ se comunican, y denotamos por $i \leftrightarrow j$.
...
\subsection*{Definición:}
Se dice que una cadena de Markov es irreducible si existe una única clase, es decir, todos los estados se comunican entre sí.
...
$f_{00}^{(n)} = \frac{1}{n} \times \frac{1}{n-1}$ para $n \geq 2$
...
0 es recurrente nulo y como $j \leftrightarrow 0$; $j \in S$ entonces todos los estados son recurrentes nulos.
...
Considere la distribución inicial $\pi_0 = (0.2, 0.8)$ y la matriz de transición:
...
Si la cadena de Markov es irreducible y sus estados son recurrentes positivos, entonces la medida estacionaria $\pi$ existe y es única. Además:
